...
import { generateChat, DEFAULT_CHAT, TITLE, sumitFirstStageReport, submitSecondStageReport, getChatHistory} from '../utils/openai.ts';
...
const nextStage = async () => {
  ...
  await sumitFirstStageReport(props.userID, userInput1.value, timeSpent);
...
const commit = async () => {
  if (window.confirm('点击确定后无法更改答案')) {
    ...
    await submitSecondStageReport(props.userID, userInput2.value, timeSpent); // 保存第二阶段报告
    alert('报告已提交');
